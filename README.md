...
1. `100%`开源
2. `100%`遵循Docker标准
2. 支持**多版本PHP**随意切换（PHP5.4、PHP5.6、PHP7.0、PHP7.2)
3. 支持绑定任意**多个域名**
4. 支持**HTTPS和HTTP/2**
5. PHP源代码位于host中
6. MySQL data位于host中
...
├── docker-compose70.yml    PHP7.0 docker-compose项目文件
...
$ docker-compose -f docker-compose70.yml up
```
...
$ docker-compose -f docker-compose70.yml up --build
```
...
### 7.2 phpRedisAdmin
phpRedisAdmin容器映射到主机的端口地址是：`8082`，所以主机上访问phpMyAdmin的地址是：
```
http://localhost:8082
```
